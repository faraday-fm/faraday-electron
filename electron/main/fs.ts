...
async function listDir(dirPath: string, signal: AbortSignal) {
...
export function initFsApi() {
  ipcMain.on(
    "fs",
    async (e, { id, operation }: { id: number; operation: FsOperation }) => {
          ...
          case "watch":
            {
              ...
              if ((await fs.stat(operation.path)).isDirectory()) {
                (await listDir(operation.path, signal)).forEach((e) =>
                  entries.push(e)
                );
              }
